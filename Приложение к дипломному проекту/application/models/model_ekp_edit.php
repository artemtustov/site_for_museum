	...
	{	
		...
		if(isset($_POST['people_search_start']))
		{
                if ($this->user_id == 'admin'){
                    $query = "SELECT * FROM kpvo_2.all WHERE kpvo_2.all.Familiya like '"
                        .$_POST['people_search_surname']."' or kpvo_2.all.Imya like '"
                        .$_POST['people_search_name']."' or kpvo_2.all.Otchestvo like '"
                        .$_POST['people_search_patronimic']."' ORDER BY id DESC;";
                $db->run($query);}
		...
		$query = "SELECT * FROM kpvo_2.all WHERE (kpvo_2.all.Familiya like '"
                        .$_POST['people_search_surname']."' or kpvo_2.all.Imya like '"
                        .$_POST['people_search_name']."' or kpvo_2.all.Otchestvo like '"
                        .$_POST['people_search_patronimic']."') and kpvo_2.all.Show = 'Yes' and kpvo_2.all.Rayon ='"
                        .$this->user_id."' ORDER BY id DESC;";
                $db->run($query);}
		...
		return $data_array;
                ...
                if(isset($_POST['people_delete_start']))
		{
		$query = "DELETE FROM kpvo_2.chelovek WHERE id ='".$_POST['people_delete']."';";
		$db->run($query);
                header("Location: http://museum.by/ekp_edit");
		}
                
                if(isset($_POST['people_edit_start']))
		{
		header("Location: http://museum.by/ekp_rw");
		}
